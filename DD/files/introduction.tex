...
The scope of the BBP system focuses on managing cyclists' activities and collecting information about all possible paths from every staring and arrival point. The system includes features for recording personal trips, making possibile for users to save their history and view detailed statistics enriched with external weather data. A central aspect of the scope is the inclusion of publishable information on the status of paths and the eventual presence of obstacles, which can be inserted either manually or with the automatic detection mode, the latter one uses the mobile phone's sensors to detect potholes or other possible obstacles during the activity. To ensure the reliability of the automatically collected data, the system will have mandatory validation or correction by the user before the publication of the bike path. Finally, BBP includes a "view and route" feature that allows registered or not users to select a starting point and destination in order to view all the available routes on a map, classified according to a quality and efficiency score.
